...
% Written by Yu Xiang
% --------------------------------------------------------
function NTHU_display_results

is_save = 1;

opt = globals();
N = numel(opt.nthu_test_seqs);
seq_set = 'test';
...
for seq_idx = 1:N
    ...
    filename = sprintf('results_nthu/%s.txt', seq_name);
    ...
    if is_save
        file_video = sprintf('results_nthu_videos/%s.avi', seq_name);
