...
%
% testing MDP
function MDP_test_NTHU(seq_idx, seq_set, tracker)
...
filename = sprintf('%s/%s.txt', opt.results_nthu, seq_name);
fprintf('write results: %s\n', filename);
write_tracking_results_nthu(filename, dres_track, opt.tracked);

% save results
if is_save
    filename = sprintf('%s/nthu_%s_%s_results.mat', opt.results_nthu, seq_set, seq_name);
