...
%
% training MDP
function tracker = MDP_train_NTHU(seq_idx, tracker)
...
[dres_train, dres_det, labels] = generate_training_data_nthu(seq_idx, dres_image, opt);

if isempty(dres_train)
    return;
end
...
while 1
    ...
    if isempty(find(is_good == 0, 1)) == 1
        ...
        if count == opt.max_pass
            ...
            % check if all the sequences are difficult
            if isempty(find(is_good == 0, 1))
                break;
            end
        end
    end
