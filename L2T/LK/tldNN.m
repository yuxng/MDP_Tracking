...
for i = 1:size(x,2) % fore every patch that is tested
    ...
    % set isin
    if any(nccP > tld.ncc_thesame), isin(1,i) = 1;  end % IF the query patch is highly correlated with any positive patch in the model THEN it is considered to be one of them
    [~,isin(2,i)] = max(nccP); % get the index of the maximall correlated positive patch
    if any(nccN > tld.ncc_thesame), isin(3,i) = 1;  end % IF the query patch is highly correlated with any negative patch in the model THEN it is considered to be one of them
    ...
    maxP = max(nccP(1:ceil(tld.valid*size(tld.pex,2))));
    dP = 1 - maxP;
    conf2(i) = dN / (dN + dP);
